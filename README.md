...
2. **Tham Gia Nhanh Không Cần Đăng Ký**: Bạn bè khi nhận link chia sẻ chỉ cần nhập biệt danh hài hước (ví dụ: *Chiến Thần Diệt Mồi, Beer Thủ Vô Song*) là tham gia sòng nhậu ngay lập tức.
...
*(Trình duyệt của bạn sẽ tự động kết nối API & WebSockets chéo cổng nhờ cấu hình proxy trong `vite.config.js`)*
...
3. **Lấy địa chỉ IP máy tính của bạn**:
   * Trên macOS: Mở *System Settings -> Wi-Fi -> Details* hoặc gõ `ipconfig getifaddr en0` trong Terminal. Ví dụ IP của bạn là `192.168.1.15`.
...
*BeerVote - Kết nối chiến hữu, cạn ly rực rỡ!*
