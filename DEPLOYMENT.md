...
- **100% CPU Spike / PM2 Crash Loop (`EADDRINUSE`):** If your VM CPU hits 100% and `pm2 status` shows a process with hundreds of restarts, you accidentally started multiple PM2 instances in `fork` mode. The second instance will crash repeatedly trying to bind to the same port. To fix, delete the duplicate instance (`pm2 delete <id>`). If you want to run multiple instances intentionally, you must use PM2 Cluster Mode (`-i max`).
- **Linux OOM (Out of Memory) Kills:** Node.js can easily consume all RAM on a 1GB memory VM. Run `sudo fallocate -l 1G /swapfile`, `sudo chmod 600 /swapfile`, `sudo mkswap /swapfile`, and `sudo swapon /swapfile` to create emergency overflow memory. Make it permanent by running `echo '/swapfile none swap sw 0 0' | sudo tee -a /etc/fstab`.
