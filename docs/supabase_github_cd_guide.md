...
| :--- | :--- |
| **`SUPABASE_ACCESS_TOKEN`** | Khóa truy cập cá nhân của bạn (Lấy trên Supabase Dashboard tại *Account Settings ➔ Access Tokens ➔ Generate new token*). Dùng chung cho cả 2 môi trường. |
| **`SUPABASE_PROJECT_REF_DEV`** | Mã Reference ID của dự án **Supabase Dev** (Lấy tại *Project Settings ➔ General ➔ Reference ID*). |
...
```
*Hệ thống sẽ tạo ra một file trống tại: `supabase/migrations/<timestamp>_ten_tinh_nang_moi.sql`*

### Bước 2: Viết mã SQL thay đổi cấu trúc
Mở file SQL vừa được tạo ra và viết câu lệnh SQL thay đổi cấu trúc dữ liệu của bạn.
*Ví dụ (Thêm cột avatar vào bảng guests):*
```sql
