...
1. **Two Redis Connections**: Every Node.js instance creates _two_ connections to Redis: one for publishing messages (`pubClient`) and one strictly for listening/subscribing (`subClient`).
...
4. **Broadcasting**: When an instance receives a message from Redis, it broadcasts that payload to all of its _locally connected_ WebSocket clients.
